...
import { defaultEdits, type StampEdits, type TextItem } from '../types'
import { applyMask, createCanvas, ctx2d, urlToImage } from '../lib/imageUtils'
import { TEXT_FONT_FAMILY } from '../lib/compose'
 ...
 * - テキストの追加・ドラッグ配置
 * - 白フチのオン/オフ
 * プレビューは 元画像×マスク の合成をフィット表示し、テキストは同キャンバスに重ね描きする。
 */
export function EditorModal() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setEdits(structuredClone(item.edits))
      setReady(true)
    })()
    return () => {
      cancelled = true
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4" data-testid="editor-modal">
      <div className="flex max-h-full w-full max-w-2xl flex-col overflow-y-auto rounded-2xl bg-white p-4 shadow-xl">
        ...
        {/* プレビューキャンバス */}
        <div className="checkerboard mx-auto rounded-lg" style={{ touchAction: 'none' }}>
          {ready ? (
            <canvas
              ref={canvasRef}
        ...
        {edits.outline && (
          <p className="mt-1 text-center text-xs text-slate-400">
            白フチは書き出し時に付与されます(このプレビューには表示されません)
          </p>
        )}
